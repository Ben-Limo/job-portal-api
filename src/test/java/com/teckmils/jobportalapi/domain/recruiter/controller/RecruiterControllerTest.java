...
class RecruiterControllerTest {
    ...
    @Test
    void recruiterWhenUnauthenticatedThenReturns401() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/recruiter"))
                .andExpect(status().isUnauthorized());
    }
    @Test
    void recruiterReturnsRecruiterDetails() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/recruiter")
                        .with(user("mockUser")))
                .andExpect(status().isOk())
                .andExpect(content().string("Here are the recruiter details"));
    }
